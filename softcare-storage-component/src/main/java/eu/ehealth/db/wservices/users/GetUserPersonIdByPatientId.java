 ...
 *
 */
public class GetUserPersonIdByPatientId extends DbStorageComponent<OperationResult, Object>
{
	 ...
	 * @param session
	 */
	public GetUserPersonIdByPatientId(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			StorageComponentMain.scLog("DEBUG", "GetUserIdByPatientId idv   : " + idv);
			StorageComponentMain.scLog("DEBUG", "GetUserIdByPatientId userId: " + userId);
			...
			try
			{
				...
				String sql = "SELECT a.personId FROM aladdinuser a LEFT JOIN patient p ON p.persondata = a.personId WHERE p.id = " + uid.toString();
