 ...
 *
 */
public class GetUserIdByCarerId extends DbStorageComponent<OperationResult, Object>
{
	 ...
	 * @param session
	 */
	public GetUserIdByCarerId(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			try
			{
				...
				String sql = "SELECT a.id FROM aladdinuser a LEFT JOIN carer c ON c.persondata = a.personId WHERE c.id = " + uid.toString();
