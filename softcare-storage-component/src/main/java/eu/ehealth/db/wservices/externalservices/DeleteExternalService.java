 ...
 *
 */
public class DeleteExternalService extends DbStorageComponent<OperationResult, String>
{
	 ...
	 * @param session
	 */
	public DeleteExternalService(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<String> lo)
	{
		try
		{
			...
			// DEBUG - TRACE
			StorageComponentMain.scLog("DEBUG", "METHOD : DeleteExternalService");
			...
			try
			{
				...
				_session.createSQLQuery("DELETE FROM externalservice WHERE id = " + id.toString()).executeUpdate();
