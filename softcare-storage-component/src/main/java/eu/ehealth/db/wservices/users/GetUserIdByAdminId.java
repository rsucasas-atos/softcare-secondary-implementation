 ...
 *
 */
public class GetUserIdByAdminId  extends DbStorageComponent<OperationResult, Object>
{
	 ...
	 * @param session
	 */
	public GetUserIdByAdminId(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			if (!checkUserPermissions("", userId, U_SERVICE, U_ADMIN))
			{
			...
			try
			{
				...
				String sql = "SELECT a.id FROM aladdinuser a LEFT JOIN administrator c ON c.persondata = a.personId WHERE c.id = " + uid.toString();
