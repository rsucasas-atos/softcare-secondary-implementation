 ...
 *
 */
public class DeleteClinician extends BaseAppUsersOperations
{
	 ...
	 * @param session
	 */
	public DeleteClinician(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			try
			{
				...
				eu.ehealth.db.db.Clinician p = 
						(eu.ehealth.db.db.Clinician) _session.load(eu.ehealth.db.db.Clinician.class, id);
				Integer pd = p.getPersondata();

				_session.createSQLQuery("DELETE FROM identifier WHERE persondata = (SELECT persondata FROM clinician WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM address WHERE persondata = (SELECT persondata FROM clinician WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM communication WHERE persondata = (SELECT persondata FROM clinician WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM clinician WHERE id = " + id.toString()).executeUpdate();
