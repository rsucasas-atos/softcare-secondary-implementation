 ...
 *
 */
public class DeleteCarer extends BaseAppUsersOperations
{
	 ...
	 * @param session
	 */
	public DeleteCarer(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			...
			try
			{
				...
				_session.createSQLQuery("DELETE FROM identifier WHERE persondata = (SELECT persondata FROM carer WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM address WHERE persondata = (SELECT persondata FROM carer WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM communication WHERE persondata = (SELECT persondata FROM carer WHERE id = "
						+ id.toString() + ")").executeUpdate();
				_session.createSQLQuery("DELETE FROM carer WHERE id = " + id.toString()).executeUpdate();
