 ...
 *
 */
public class AddMediaContent extends DbStorageComponent<OperationResult, Object>
{
	 ...
	 * @param session
	 */
	public AddMediaContent(Session session)
	{
		super(session);
	}
	...
	protected OperationResult dbStorageFunction(ArrayList<Object> lo)
	{
		try
		{
			MediaContent in = (MediaContent) lo.get(0); 
			String userId = (String) lo.get(1);
			
			// DEBUG - TRACE
			StorageComponentMain.scLog("DEBUG", "METHOD : AddMediaContent");			
			...
			try
			{
				...
				Integer savedId = fm.importMediaContent(in, null);
